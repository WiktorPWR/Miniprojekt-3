...
int main() {
    ...
    for (int elemnty : ilosc_elemtow) {
        for (int i = 0; i <= ilosc_powtorzen; i++) {
            ...
            auto elapsed_insert_chain = std::chrono::duration_cast<std::chrono::nanoseconds>(end_insert_robin_hood - begin_insert_robin_hood);
            time_insert_robin_hood += elapsed_insert_robin_hood.count();
            ...
            auto elapsed_remove_chain = std::chrono::duration_cast<std::chrono::nanoseconds>(end_remove_robin_hood - begin_remove_robin_hood);
            time_remove_chain += elapsed_remove_robin_hood.count();
